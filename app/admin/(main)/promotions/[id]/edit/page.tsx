...
import { usePromotion, useUpdatePromotion, useProducts, useServices } from '@/hooks';
...
export default function EditPromotionPage() {
  ...
  // Promotion is automatically fetched by React Query
  ...
  useEffect(() => {
    if (promotion) {
      ...
      setFormData({
        ...
        productsIds: promotion.productsIds || [],
        servicesIds: promotion.servicesIds || [],
        image: promotion.image || '',
      });
      setImagePreview(promotion.image || '');
      setSlugManuallyEdited(!!promotion.slug);
  ...
  if (promotionLoading || initialLoad || !promotion) {
    return (
  ...
  if (!promotion) {
    return (
      <div className="text-center py-8 sm:py-12">
        <p className="text-sm sm:text-base text-text-secondary mb-4">Promotion not found</p>
        <Link href="/admin/promotions">
          <Button variant="outline" size="sm" className="sm:size-default">Back to Promotions</Button>
        </Link>
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="flex items-center gap-3 sm:gap-4 mb-6 sm:mb-8">
        ...
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-foreground">Edit Promotion</h1>
